...
export const actionTypes = {
  ...
  START_ALL_COMPLETE: 'START_ALL_COMPLETE',
  START_ALL_FAILED: 'START_ALL_FAILED',
  START_ALL: 'START_ALL'
};
...
export function startAllComplete () {
  return {
    type: actionTypes.START_ALL_COMPLETE
  };
}

export function startAllFailed (err) {
  return {
    type: actionTypes.START_ALL_FAILED,
    err
  };
}
...
export function startAll () {
  return dispatch => {
    dispatch({
      type: actionTypes.START_ALL
    });

    superagent.get('/api/cameracoders/all/start')
      .accept('application/json')
      .end((err, res) => {
        if (err) {
          dispatch(smessagesActions.openMessage('Ошибка: ' + err.message + '. Запись со всех камер не запущена.'));
          dispatch(startAllFailed(err));
        } else {
          dispatch(smessagesActions.openMessage('Включена запись со всех камер.'));
          dispatch(startAllComplete());
          dispatch(loadCameraCoders());
        }
      });
  };
